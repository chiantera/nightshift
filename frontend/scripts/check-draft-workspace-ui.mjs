...
assert.match(domainTypes, /export type TabId = 'timeline' \| 'deadlines' \| 'facts' \| 'analisi' \| 'piani' \| 'questions' \| 'brief'/);
assert.match(cdv, /function DraftingWorkspace/);
// Draft creation now routes through the pre-flight "istruzioni per Aria" modal.
...
assert.equal(/Prepara con Aria[\s\S]{0,260}onOpenChat/.test(cdv), false, 'deadline preparation button must not open chat');
...
assert.match(cdv, /Proteggi tutto come \.spr/);
